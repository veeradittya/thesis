            ...
            <div className="flex flex-1 items-center justify-end gap-4 pr-4 sm:pr-[25px]">
              {session?.user ? (
                <div className="relative">
                  {/* Initial-letter avatar (not the Google photo) → account dropdown with sign out. */}
                  <button
                    onClick={() => setAcctMenu((v) => !v)}
                    title={session.user.email ?? session.user.name ?? "Account"}
                    className="grid h-10 w-10 place-items-center rounded-full bg-white/10 text-[13px] font-medium text-white ring-1 ring-white/15 transition-colors hover:ring-white/40"
                  >
